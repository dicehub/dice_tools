...
class JsonOrderedDict(OrderedDict):
    """
    An OrderedDict replacement which loads and writes the data from and to a json file.
    """
    ...
    def write(self):
        """
        Writes data to file. If write_signal is not None calls it after data is
            written.
        """
...
class YamlOrderedDict(OrderedDict):
    """
    An OrderedDict replacement which loads and writes the date from and to a
    yaml file.

    Args:
        file_name (str): path to YAML file.
        autowrite: if True data changes in class instance are automatically
            synchronized with YAML file.
        write_signal: function to call on data changes are written to YAML file.
    """
    def __init__(self, file_name, autowrite=True, write_signal=None):
        super().__init__()
        self.file_name = file_name
    ...
    def __load_file(self):
        ...
        try:
            ...
            if not isinstance(e, FileNotFoundError):
                s = 'Cannot read from "%s":' % self.file_name
                print(s, format_exc())
        finally:
            self.__loading = False
    ...
    def write(self):
        """
        Writes data to file. If *write_signal* is not None calls it after data
        is written.
        """
        try:
            d = {}
            for k, v in self.items():
                d[k] = v
            if self.file_name:
                s = yaml.dump(d, width=50, indent=4, default_flow_style=False)
                with open(self.file_name, "w") as f:
                    f.write(s)
            else:
                yaml.dump(d, self.stream, width=50, indent=4,
